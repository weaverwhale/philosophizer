...
export function AdminPage() {
  ...
  const handleReseedRAG = () => {
    if (!confirm('Re-seeding will take several minutes. Continue?')) {
      return;
    }
  ...
  const handleRestoreBackup = () => {
    ...
    if (
      !confirm(
        `Restore from ${selectedBackup}? This will replace all current RAG data.`
      )
    ) {
      return;
    }
          ...
          <div className="bg-surface border border-border rounded-lg p-4">
            <div className="flex gap-3 mb-4">
              ...
              <button
                onClick={loadStats}
                disabled={operationInProgress}
                className="px-4 py-2 bg-surface border border-border text-text rounded-md hover:bg-surface-hover disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
              >
                Refresh Stats
              </button>
            </div>
